...
describe('Campaigns (e2e)', () => {
  ...
  it('rejects an end date before the start date', async () => {
    ...
    const res = await request(server())
      .post('/api/v1/campaigns')
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ ...basePayload, tripEndDate: '2026-08-01', photoMediaIds: [mediaId] })
      .expect(400);
  ...
  it('rejects more than 5 photos', async () => {
    ...
    await request(server())
      .post('/api/v1/campaigns')
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ ...basePayload, photoMediaIds: ['m1', 'm2', 'm3', 'm4', 'm5', 'm6'] })
      .expect(400);
  });
  ...
  describe('public browse (GET /campaigns)', () => {
    it('lists only public campaigns, excluding private ones from anyone but their creator', async () => {
      ...
      await request(server())
        .post('/api/v1/campaigns')
        .set('Authorization', `Bearer ${alice.accessToken}`)
        .send({ ...basePayload, title: 'Public Trip', privacy: 'public', photoMediaIds: [mediaId] })
        .expect(201);
      await request(server())
        .post('/api/v1/campaigns')
        .set('Authorization', `Bearer ${alice.accessToken}`)
        .send({ ...basePayload, title: 'Private Trip', privacy: 'private', photoMediaIds: [mediaId] })
        .expect(201);
    ...
    it('filters by creatorId, and by title/destination search text', async () => {
      const alice = await registerAndVerifyTraveler(server, 'alice10@e2e.test', 'Alice');
      const bob = await registerAndVerifyTraveler(server, 'bob10@e2e.test', 'Bob');
      ...
      await request(server())
        .post('/api/v1/campaigns')
        .set('Authorization', `Bearer ${alice.accessToken}`)
        .send({ ...basePayload, title: 'Santorini Dream', destination: 'Santorini, Greece', photoMediaIds: [aliceMedia] })
        .expect(201);
      await request(server())
        .post('/api/v1/campaigns')
        .set('Authorization', `Bearer ${bob.accessToken}`)
        .send({ ...basePayload, title: 'Bali Adventure', destination: 'Ubud, Bali', photoMediaIds: [bobMedia] })
        .expect(201);
      ...
      const byCreatorRes = await request(server())
        ...
        .set('Authorization', `Bearer ${bob.accessToken}`)
        .expect(200);
      expect(byCreatorRes.body.data.items.map((c: any) => c.title)).toEqual(['Santorini Dream']);

      const bySearchRes = await request(server())
        .get('/api/v1/campaigns?search=bali')
        .set('Authorization', `Bearer ${alice.accessToken}`)
        .expect(200);
      expect(bySearchRes.body.data.items.map((c: any) => c.title)).toEqual(['Bali Adventure']);
    });
  });
});
