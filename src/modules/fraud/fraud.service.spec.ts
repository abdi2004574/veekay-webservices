...
describe('FraudService', () => {
  ...
  describe('checkPaymentMethodMismatch', () => {
    it('creates flag when name does not match', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: 'user-1',
        displayName: 'Alice Smith',
        travelerProfile: { id: 'tp-1' },
      });
      ...
      prisma.fraudFlag.create.mockResolvedValue({
        ...
        type: FraudFlagType.payment_method_mismatch,
        severity: FraudFlagSeverity.medium,
        description: 'Payment details mismatch: name.',
        status: FraudFlagStatus.open,
        reviewedById: null,
        reviewedAt: null,
        resolutionNote: null,
      ...
      const result = await service.checkPaymentMethodMismatch('user-1', {
        nameOnCard: 'Bob Jones',
      });

      expect(result).not.toBeNull();
      expect(result?.type).toBe(FraudFlagType.payment_method_mismatch);
      expect(prisma.fraudFlag.create).toHaveBeenCalled();
    });

    it('returns null when details match', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: 'user-1',
        displayName: 'Alice Smith',
        travelerProfile: { id: 'tp-1' },
  ...
  describe('checkWithdrawalAnomaly', () => {
    it('creates flag for high-value withdrawal', async () => {
      prisma.withdrawalRequest.findMany.mockResolvedValue([
        { id: 'wd-1', status: 'approved' },
        { id: 'wd-2', status: 'paid' },
      ]);
    ...
    it('returns null for below-threshold withdrawal', async () => {
      const result = await service.checkWithdrawalAnomaly('user-1', 500, 'USD');
